...
if ($rowJumlahKasusTotal['jumlah_total'] == 0) {
  ...
  $hapus1 = mysqli_query($koneksi, "TRUNCATE hitung_uji");
  $hapus2 = mysqli_query($koneksi, "TRUNCATE gausian");
  ...
  /////////////////////////////
  $ambildata  = mysqli_query($koneksi, "SELECT * FROM data_uji ORDER BY id");
  while ($data1  = mysqli_fetch_array($ambildata)) {
    $id_uji = $data1['id'];
...
('','$data1[id]','iterasi kelas Puas','$jumlah_mk1','$absensi1','$sks_s11','$sks_s21','$sks_s31','$sks_s41','$ips_s11','$ips_s21','$ips_s31','$ips_s41'),
('','$data1[id]','iterasi kelas Tidak Puas','$jumlah_mk2','$absensi2','$sks_s12','$sks_s22','$sks_s32','$sks_s42','$ips_s12','$ips_s22','$ips_s32','$ips_s42')";
  ...
  $ambildata10  = mysqli_query($koneksi, "SELECT * FROM hitung_uji WHERE atribut ='uji' ORDER BY id");
  while ($data10 = mysqli_fetch_array($ambildata10)) {
    $id_data = $data10['id'];
  ...
  echo "<script type='text/javascript'>
      setTimeout(function () {  
        ...
        window.setTimeout(function(){ 
        window.location.replace('konten.php?page=result-classification');
      } ,3000);
  </script>";
}
